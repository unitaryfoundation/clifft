...
def assert_forced_record_probabilities(
    ...
    gpu_sampler: cuda.Sampler,
    *,
    absolute_tolerance: float,
) -> None:
    """Enumerate every small record branch and compare reachability and probability."""
    num_records = gpu_sampler.program.num_records
    assert num_records == cpu_program.num_measurements + cpu_program.num_hidden_measurements
    if cpu_program.num_hidden_measurements != 0:
        raise ValueError(
            "forced-record enumeration needs a program without hidden records, because "
            "record_probabilities accepts only visible measurement columns"
        )
    ...
    for record, log_probability in zip(records, cpu_log_probabilities, strict=True):
        replay = gpu_sampler.replay_shot(record.tolist())
