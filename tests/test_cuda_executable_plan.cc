...
using clifft::sampling::cuda::ExecutablePlan;
using clifft::sampling::cuda::detail::ActionTag;
...
TEST_CASE("CUDA sampling has an independent random stream domain") {
    STATIC_REQUIRE(clifft::kCudaSamplingExecutorDomain != clifft::kSamplingExecutorDomain);
    STATIC_REQUIRE(clifft::kCudaSamplingExecutorDomain != clifft::kHipSamplingExecutorDomain);
    STATIC_REQUIRE(clifft::kCudaSamplingExecutorDomain != clifft::kBatchSamplingExecutorDomain);
}

TEST_CASE("CUDA device layout sizes coefficient workspace consistently") {
    STATIC_REQUIRE(clifft::sampling::cuda::detail::coefficient_state_capacity(0) == 1);
    STATIC_REQUIRE(clifft::sampling::cuda::detail::coefficient_scratch_capacity(0) == 1);
    STATIC_REQUIRE(clifft::sampling::cuda::detail::coefficient_elements_per_shot(0) == 4);
    STATIC_REQUIRE(clifft::sampling::cuda::detail::coefficient_state_capacity(4) == 16);
    STATIC_REQUIRE(clifft::sampling::cuda::detail::coefficient_scratch_capacity(4) == 8);
    STATIC_REQUIRE(clifft::sampling::cuda::detail::coefficient_elements_per_shot(4) == 48);
    STATIC_REQUIRE(clifft::sampling::cuda::detail::kReductionScratchBytes ==
                   2 * clifft::sampling::cuda::detail::kMaxBlockSize * sizeof(double));
}

TEST_CASE("CUDA executable lowers existing sampling action names") {
    ...
    REQUIRE((executable.actions()[2].flags & clifft::sampling::cuda::detail::kRecordParity) != 0);
    ...
    REQUIRE((executable.actions()[4].flags & clifft::sampling::cuda::detail::kRecordParity) != 0);
    REQUIRE(executable.num_exp_vals() == 1);
}

TEST_CASE("CUDA executable inspection identifies packed modification points") {
    ...
    REQUIRE_THAT(diagnostic, ContainsSubstring("CUDA executable: actions="));
...
TEST_CASE("CUDA executable preserves selected output representations") {
    const SamplingPlan plan = plan_from(R"(
        X_ERROR(1) 1
        X_ERROR(1) 0
        M 0
    ...
    REQUIRE((historical.flags & clifft::sampling::cuda::detail::kRecordParity) == 0);
    ...
    REQUIRE((current.flags & clifft::sampling::cuda::detail::kRecordParity) != 0);
...
TEST_CASE("CUDA executable packs affine terms and categorical noise") {
    SamplingPlan plan;
...
TEST_CASE("CUDA executable flattens shared Pauli preparation") {
    const SamplingPlan plan = plan_from(R"(
        R_X(0.13) 0
        R_Y(-0.27) 1
        R_PAULI(0.31) X0*Y1
...
TEST_CASE("CUDA executable rejects work outside every device tier") {
    using Catch::Matchers::ContainsSubstring;

    SamplingPlan wide;
    wide.num_qubits = clifft::sampling::cuda::kMaxActiveWidth + 1;
    wide.initial_active_width = clifft::sampling::cuda::kMaxActiveWidth + 1;
    wide.peak_active_width = clifft::sampling::cuda::kMaxActiveWidth + 1;
...
TEST_CASE("CUDA executable accepts cultivation cooperative width") {
    ...
    REQUIRE(plan.peak_active_width > clifft::sampling::cuda::kThreadPerShotMaxActiveWidth);
    const ExecutablePlan executable(plan);
