...
std::vector<std::vector<double>> always_leaked() {
    auto m = zeros5();
    m[kLeakG][0] = 1.0;
    m[kLeakG][1] = 1.0;
    return m;
...
std::vector<std::vector<double>> always_to_g() {
    auto m = zeros5();
    m[0][0] = 1.0;
    m[0][1] = 1.0;
    return m;
...
std::vector<std::vector<double>> always_to_e() {
    auto m = zeros5();
    m[1][0] = 1.0;
    m[1][1] = 1.0;
    return m;
...
TEST_CASE(
    ...
    NonComputationalModel one = make_model(all_g(), transitions, make_classifier({0.0, 1.0}));
    ...
    NonComputationalModel zero =
        make_model(all_g(), std::move(transitions), make_classifier({1.0, 0.0}));
...
TEST_CASE("sample_noncomputational: a partial classifier bit matches its frequency") {
    ...
    NonComputationalModel model =
        make_model(all_g(), std::move(transitions), make_classifier({0.5, 0.5}));
...
TEST_CASE("sample_noncomputational: a substochastic classifier column rejects at construction") {
    // A partially substochastic leak_g column (sums to 0.8) reserves reject
    // probability, which no sampling path supports; the model refuses to
    // build rather than failing on whichever shot first classifies.
    std::map<std::string, std::vector<std::vector<double>>> partial;
    partial.emplace("S", always_leaked());
    REQUIRE_THROWS_WITH(make_model(all_g(), std::move(partial), make_classifier({0.5, 0.3})),
                        ContainsSubstring("reject columns are not supported"));

    // A fully reject column (sums to 0) is refused the same way.
    std::map<std::string, std::vector<std::vector<double>>> empty;
    empty.emplace("S", always_leaked());
    REQUIRE_THROWS_WITH(make_model(all_g(), std::move(empty), make_classifier({0.0, 0.0})),
                        ContainsSubstring("reject columns are not supported"));
}
...
TEST_CASE("sample_noncomputational: a measure-and-reset on a leaked qubit injects and restores") {
    ...
    transitions.emplace("S", always_leaked());
    NonComputationalModel model =
        make_model(all_g(), std::move(transitions), make_classifier({0.0, 1.0}));
...
TEST_CASE(
    ...
    NonComputationalModel model =
        make_model(all_g(), std::move(transitions), classifier_with(kLost, {0.0, 1.0}));
...
TEST_CASE("sample_noncomputational: a jump to the ground level forces the measurement to 0") {
    ...
    Circuit c = parse("H 0\nS 0\nM 0\n");
    std::map<std::string, std::vector<std::vector<double>>> transitions;
    transitions.emplace("S", always_to_g());
...
TEST_CASE("sample_noncomputational: a jump to the excited level forces the measurement to 1") {
    Circuit c = parse("H 0\nS 0\nM 0\n");
    std::map<std::string, std::vector<std::vector<double>>> transitions;
    transitions.emplace("S", always_to_e());
    NonComputationalModel model = make_model(all_g(), std::move(transitions));
...
TEST_CASE("sample_noncomputational: recapturing a lost qubit clears the stale residual") {
    ...
    transitions.emplace("M", std::move(m));
    NonComputationalModel model =
        make_model(all_e(), std::move(transitions), classifier_with(kLost, {1.0, 0.0}));
