...
nb::object rows_to_python(clifft::sampling::SamplingResult result, uint32_t rows,
                          const clifft::sampling::cuda::Sampler& sampler) {
    auto measurements =
...
nb::object survivors_to_python(clifft::sampling::SamplingSurvivorResult result,
                               const clifft::sampling::cuda::Sampler& sampler, bool keep_records) {
...
NB_MODULE(_clifft_cuda, module) {
    ...
    module.doc() = "Experimental Clifft CUDA backend";
    ...
    module.def(
        ...
        [](const ExecutablePlan& program, CoefficientPrecision precision) {
            nb::gil_scoped_release release;
            return clifft::sampling::cuda::selected_tier(program, precision);
        },
    ...
    nb::class_<Sampler>(module, "Sampler")
        .def(nb::init<const ExecutablePlan&, CoefficientPrecision, uint32_t, ExecutionTier,
                      uint32_t>(),
             nb::arg("program"), nb::arg("coefficient_precision") = CoefficientPrecision::FP64,
             nb::arg("max_batch_shots") = clifft::sampling::cuda::kDefaultMaxBatchShots,
             nb::arg("tier") = ExecutionTier::Auto, nb::arg("max_concurrent_shots") = 0)
        ...
        .def_prop_ro("max_concurrent_shots",
                     [](const Sampler& sampler) { return sampler.max_concurrent_shots(); })
        ...
        .def(
            ...
            nb::arg("block_size") = clifft::sampling::cuda::kDefaultBlockSize)
        .def(
            "sample_survivors",
        ...
        .def(
            ...
            [](Sampler& sampler, std::vector<uint8_t> forced_records) {
                clifft::sampling::cuda::ReplayResult result;
                {
                    nb::gil_scoped_release release;
    ...
    module.def("is_available", &clifft::sampling::cuda::is_available);
    module.def("backend_info", &clifft::sampling::cuda::backend_info);
}
