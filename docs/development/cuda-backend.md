...
The CUDA backend selects one of three tiers per program and device:
...
The per-shot RNG derives from the seed root and global shot index, so a shot
draws the same random stream whatever the batch size and concurrency cap.
Rows are reproducible for a fixed tier, precision, and block size. Rows are *not*
...
Each shared kernel opts into the device's maximum shared-memory budget once
during construction. Individual launches request only the bytes their program
needs, so samplers with different widths can safely use the same kernel
concurrently.
...
CPU and CUDA use separate random-stream domains, so compare
deterministic branches directly and stochastic results statistically.

## Performance considerations

Tier selection balances coefficient storage and the amount of work per shot.
The width-four thread-per-shot cutoff is conservative: its global coefficient
pool grows with both batch size and active width, while the shared tier keeps
coefficients on chip. A different crossover on one circuit does not establish
a better default for all workloads.

The cooperative tiers launch up to 32 blocks per multiprocessor by default,
subject to batch size and, for global coefficient storage, available memory.
This oversubscribes the device so its scheduler can fill available capacity;
it does not imply that all launched blocks reside on the multiprocessors at
once. Each global block needs its own coefficient slab, so increasing the cap
also increases retained storage.

Early postselection failures can make per-shot control overhead dominate the
coefficient work. The automatic policy uses peak active width and memory fit,
so it does not account for discard depth. Measure the relevant workload and
...
alternatives: host-computed Pauli phases and pairings, active-width
transitions, expressions, and noise distributions. The device executes the
plan without topology planning or allocation in its dispatch loop. CUDA
lowering shares execution-ready Pauli preparation and result containers with
the CPU backend; its executable layout, workspace, and kernels remain
backend-specific.
...
cooperative concurrency cap, separate samplers called concurrently, and
automatic selection on both sides of the device's shared-memory boundary
(located at runtime through
