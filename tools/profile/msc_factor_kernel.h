// Compiled CSS contraction arithmetic shared by standalone research workers.
#pragma once
#include <algorithm>
#include <array>
#include <bit>
...
struct Case {
    unsigned terms = 0;
    std::array<unsigned, MAX_TERMS> labels{};
    ...
    Case(Reader& r, unsigned width, unsigned rank) {
        ...
        auto read_labels = r.ints();
        require(read_labels.size() == terms);
        for (unsigned k = 0; k < terms; ++k) {
            require(read_labels[k] < (uint64_t{1} << rank));
            labels[k] = read_labels[k];
        }
