...
def build_environment(repo_root: Path, cpu: int, perf_path: Path) -> dict[str, Any]:
    ...
    cmake_caches = []
    for path in sorted(repo_root.glob("build*/**/CMakeCache.txt")):
        selected = []
...
def main() -> None:
    ...
    parser.add_argument(
        "--expected-commit",
        default="3fdafa411e8e4eca812ff17df2a8f30c584fdc03",
    )
    ...
    if git_commit != args.expected_commit:
        raise RuntimeError(f"unexpected Clifft commit: {git_commit} != {args.expected_commit}")
    document: dict[str, Any] = {
