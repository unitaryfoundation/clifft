...
enum class Mode { Survivors, ImportanceK0, Noncomputational, Compile };
...
Mode parse_mode(std::string_view value) {
    ...
    throw std::invalid_argument("mode must be survivors, importance-k0, noncomp, or compile");
}
...
clifft::CompiledModule compile_legacy(const std::string& text) {
...
int main(int argc, char** argv) {
    ...
    try {
        ...
        if (mode == Mode::Compile) {
            ...
            const clifft::CompiledModule program = compile_legacy(text);
            ...
            const clifft::sampling::ExecutablePlan program = compile_symbolic(text);
