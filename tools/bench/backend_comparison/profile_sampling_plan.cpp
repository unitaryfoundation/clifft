...
int main(int argc, char** argv) {
    ...
    try {
        ...
        uint64_t dynamic_rotation_signs = 0;
        uint64_t dynamic_rotation_coefficient_visits = 0;
        uint64_t constant_negative_rotation_signs = 0;
        ...
        for (const auto& planned : plan.actions) {
            ...
            if (const auto* rotation =
                ...
                dynamic_rotation_signs += !rotation->sign.terms().empty();
                dynamic_rotation_coefficient_visits +=
                    !rotation->sign.terms().empty() ? uint64_t{1} << planned.active_before : 0;
                constant_negative_rotation_signs +=
                    rotation->sign.terms().empty() && rotation->sign.constant();
            }
        ...
        std::cout << "  \"predicted_coefficient_visits\": " << predicted_coefficient_visits
                  << ",\n";
        std::cout << "  \"dynamic_rotation_signs\": " << dynamic_rotation_signs << ",\n";
        std::cout << "  \"dynamic_rotation_coefficient_visits\": "
                  << dynamic_rotation_coefficient_visits << ",\n";
        std::cout << "  \"constant_negative_rotation_signs\": " << constant_negative_rotation_signs
                  << ",\n";
