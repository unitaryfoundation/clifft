...
\begin{frame}{A \,\textperiodcentered\, Where it wins, where it doesn't}
  ...
  \begin{tabular}{@{}p{0.30\textwidth}p{0.30\textwidth}p{0.30\textwidth}@{}}
    ...
    \textcolor{copper}{\textbf{Frame pays off}} once Clifford depth $\gtrsim$ block width ($8\times$ and growing past the boundary); the standard open-source tool cannot run these circuits at all. \\
    \bottomrule
  \end{tabular}
  \vspace{0.8em}
  \begin{itemize}
    \item Also measured where it \emph{loses}: at low Clifford depth the frame's bookkeeping costs $\sim\!2\times$ --- the advantage is a measured phase boundary, not a slogan.
  \end{itemize}
  \takeaway{Status: validated prototype (Python + C++), honest benchmark note; next --- one more external baseline and a natural adaptive workload, then a methods paper.}
\end{frame}
...
\begin{frame}{How the two bets fit together}
  ...
  \end{itemize}
  \vspace{0.4em}
  {\small\color{mutedink}\textbf{Next:} A --- external baseline (Quokka\#) + a natural adaptive workload, then a methods paper. \quad B --- the GH200 run, then batched replay queries.}
\end{frame}
