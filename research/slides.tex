...
\begin{frame}{A \,\textperiodcentered\, How it fits: one compiler, two executors}
  ...
  \begin{itemize}
    ...
    \item \punch{Wired, not just proposed}: the backend runs clifft's optimized circuits; exact clifft stays the default, the backend is an explicit opt-in with an error dial $\delta$.
  \end{itemize}
...
\begin{frame}{A \,\textperiodcentered\, It generalizes: any Clifford$+T$ circuit}
  \begin{itemize}
    \item Circuits with magic \emph{everywhere}? Teleport each in-line $T$ onto a helper qubit --- all magic becomes \punch{one up-front layer} again, with \punch{no penalty for interleaving}.
...
\begin{frame}{A \,\textperiodcentered\, Run times at a glance}
  ...
  \begin{tabular}{@{}p{0.24\textwidth}p{0.44\textwidth}p{0.24\textwidth}@{}}
    ...
    \textcolor{copper}{\textbf{backend 86\,s}} (8\% err) vs clifft exact 398\,s\newline
    {\scriptsize MitM 85\,s, QuiZX 2188\,s (both exact)} &
    Quokka\# (timeout),\newline aer ($0/96$ in 455\,s) \\[2pt]
    dense IQP, $n{=}72$ &
    \textcolor{copper}{\textbf{backend 267\,s, 1.9\,GB}} &
    every exact method\newline($\sim$1\,TB) \\[2pt]
    hidden shift, 96 qubits &
    \textcolor{violet}{\textbf{clifft $\mu$s}} vs backend 8\,s --- dispatch picks clifft &
    --- \\[2pt]
