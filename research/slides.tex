...
\begin{frame}{A \,\textperiodcentered\, How it fits: one compiler, two executors}
  ...
  \vspace{-0.2em}
  \begin{itemize}
    \item Both cost exponents --- $2^k$ dense, $2^{0.228\,t}$ backend (why $0.228$: next slide) --- are known \punch{before anything runs}: dispatch is a one-line comparison, per program.
...
\begin{frame}{A \,\textperiodcentered\, It generalizes: any Clifford$+T$ circuit}
  \begin{itemize}
    ...
    \item Honest finding: clifft's compiler crushes many such circuits to tiny cores and answers in microseconds --- ``who wins'' is a \emph{rule}, not a race: exactly what the compile-time dispatch encodes.
  \end{itemize}
...
\begin{frame}{A \,\textperiodcentered\, Recycled magic: pay per episode, not per program}
  \begin{itemize}
    \item Long computations \punch{recycle} their magic: the active block grows, is measured out, grows again (``episodes''). Total magic grows with runtime; \punch{per-episode magic stays bounded}.
    \item Run one episode at a time, collapse \emph{exactly} at each boundary: cost $2^{\,\text{per-episode magic}}$ instead of $2^{\,\text{total magic}}$, \punch{zero extra error}. Measured: 72 qubits, 72 T's, $\le\!400$ terms --- whole-circuit would need $2^{72}$.
    \item Compressing \emph{below} that also works, gently: at $10\times$ fewer terms the same run returns a $10^{-22}$ record probability \punch{within 41\%} in a minute (unbiased; averaging sharpens it).
    \item Honest, measured boundary: for full records, harder compression degrades sharply with episode count --- the dispatcher's \punch{four strategies} price exact-per-episode accordingly.
  \end{itemize}
...
\begin{frame}{A \,\textperiodcentered\, The circuits behind the numbers}
  \begin{columns}[T]
    \begin{column}{0.48\textwidth}
      ...
      {\tiny\color{mutedink} magic on every qubit; \emph{compiles} to peak rank $n/2$ (crossover, frontier)}\\[0.9em]
    ...
    \begin{column}{0.48\textwidth}
      ...
      {\tiny\color{mutedink} in-line $T$ $\to$ ancilla $+$ measurement $+$ conditional $S$: any circuit becomes one magic layer (hidden shift, 96 qubits)}\\[0.9em]
      \begin{tikzpicture}[xscale=0.56, yscale=0.34,
          g/.style={draw, fill=paper, inner sep=1pt, minimum size=8pt, font=\tiny},
          t/.style={g, fill=copper!28}]
