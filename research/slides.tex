  ...
  \begin{columns}[T]
    ...
    \begin{column}{0.44\textwidth}
      ...
      \begin{tikzpicture}[scale=0.9]
        % frame band
        \fill[faint] (0,2.2) rectangle (5.2,3.0);
  ...
  \begin{columns}[T]
    ...
    \begin{column}{0.44\textwidth}
      ...
      \begin{tikzpicture}[scale=0.85]
        ...
        \node[font=\small,color=copper] at (4.05,0.6) {$+\ \cdots$};
  ...
  \begin{columns}[T]
    ...
    \begin{column}{0.44\textwidth}
      ...
      \begin{tikzpicture}[scale=0.82]
        \draw[->,mutedink] (0,0) -- (5.4,0) node[below left,font=\tiny,color=mutedink]{circuit size};
...
\begin{frame}{A \,\textperiodcentered\, Evidence (measured, against clifft's real fast path)}
  \begin{columns}[T]
    \begin{column}{0.55\textwidth}
      \begin{itemize}
        \item Every component validated to \punch{machine precision}; results checked against an exact independent method we built for the purpose.
        \item Head-to-head on hard dense circuits: the backend overtakes exact clifft at \punch{$n\!\approx\!34$} (coarse accuracy) and $n\!\approx\!48$ (8\% error).
...
\begin{frame}{A \,\textperiodcentered\, Where it wins, where it doesn't}
  ...
  \begin{tabular}{@{}p{0.30\textwidth}p{0.30\textwidth}p{0.30\textwidth}@{}}
    \toprule
    \textbf{Small circuits} & \textbf{Large, magic-sparse} & \textbf{Adaptive, Clifford-heavy} \\
    \midrule
    clifft exact is faster \emph{and} exact --- keep it. &
    \textcolor{copper}{\textbf{The niche.}} Only feasible method past the memory wall (approximate, dial-controlled). &
  ...
  \end{itemize}
  \takeaway{Status: validated prototype (Python + C++), honest benchmarks, natural-workload wins; next --- one more external baseline, then a methods paper.}
\end{frame}
...
\begin{frame}{A \,\textperiodcentered\, One compiler, two executors}
  ...
  \begin{tikzpicture}[scale=0.92, every node/.append style={transform shape},
    ...
    \node[sub] at (6.0,-0.55) {peak rank $k$, $T$-count $t$};
  ...
  \begin{itemize}
    ...
    \item Every compiler improvement makes \emph{both} engines cheaper.
  \end{itemize}
  \takeaway{clifft compiles; the cheaper engine runs.}
\end{frame}
...
\begin{frame}{How the two bets fit together}
  ...
  \end{itemize}
  \vspace{0.4em}
  {\small\color{mutedink}\textbf{Next:} A --- last external baseline (Quokka\#), then write the methods paper. \quad B --- the GH200 run, then batched replay queries.}
\end{frame}
